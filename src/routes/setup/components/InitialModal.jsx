...
const InitialModal = ({ user }) => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        try {
            await createServer(name, uploadName, user.id)
            setUploadName('')
            setFiles([])
        } catch (error) {
            console.log(error)
    ...
    return (
        <div>
            <button className="btn" onClick={() => document.getElementById('initial_modal').showModal()}>open modal</button>
