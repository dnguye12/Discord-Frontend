...
const ModalMessageFile = ({channel, userId}) => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        try {
            const res = await createMessage(userId, channel.id, '', uploadName)
            setUploadName('')
            setFiles([])
            document.getElementById('message_file_modal').close()
        } catch (error) {
            console.log(error)
        }
    }
    ...
    return (
        <dialog id="message_file_modal" className="modal">
...
export default ModalMessageFile